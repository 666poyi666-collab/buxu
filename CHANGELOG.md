...
- 修复安排编辑只保存分组名称、重命名后重新归类；修复删除非空分组会偷偷创建“我的计划”并搬移内容；修复删除不存在安排仍推进整个库 revision。非空分组现在拒绝删除，单项删除只影响精确 planId（BUG-075）。
...
- 修复 OWW221 亮屏后强制停在 Launcher 且拒绝普通后台恢复的问题；授予悬浮层权限后直接回到原训练任务（BUG-072）。
