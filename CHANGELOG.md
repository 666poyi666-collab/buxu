...
- 修复合成验收记录在历史/同步摘要中丢失热量、合成标记和分时步数的问题；明确该数据不会伪装为厂商系统运动会话。
- 修正生产手机 provisioning 使用正式 Cloud V3 endpoint，避免手机被写入 staging 地址后与 ChatGPT 正式链路分离。
- 修复训练计划删除后仍会回到列表的问题：一条 `splits` 带多余字段的合成训练记录会让服务端整批拒收 exchange，计划删除被同一批次拖住；现在训练记录在入库前后都会归一化到服务端接受的字段，无法表达的条目被隔离到冲突区，计划变更走独立优先通道，不再排在睡眠与历史回填之后。
- 修复计划删除被云端权威快照复活的问题：服务端对计划库做严格键集校验，本地 stage 原样上传时任一多余字段都会让整个 exchange 返回 `invalid_exchange`；现在计划库上传前投影为服务端唯一的合法形状，云端 replace 也会重新套用本机尚未确认的删除墓碑。
- 修复计划 revision 冲突后本地删除墓碑被丢弃的问题：重套墓碑后会用云端返回的最新 revision 自动重发完整计划库；云端读侧同时清理历史分组/计划脏字段，避免 ChatGPT 写操作再次被旧数据卡住。
- 修复非空分组删除项点击无反馈的问题：现在明确显示将删除的安排数量，并在二次确认后级联删除该分组及其成员，其他分组保持不变。
...
- 修复安排编辑只保存分组名称、重命名后重新归类；修复删除非空分组会偷偷创建“我的计划”并搬移内容；修复删除不存在安排仍推进整个库 revision；修复新建空分组被 UI 过滤、无法管理。非空分组现在经明确确认后级联删除，单项删除仍只影响精确 planId（BUG-075/087）。
