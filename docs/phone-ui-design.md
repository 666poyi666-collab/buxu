...
状态：第五轮信息架构重构已安装真机，API 35 的 1.0/1.3/2.0 字体矩阵通过，待 PT-026 真机大字体与 PT-029/030/032 完整回归
...
- 排版：页面当前目的地使用 24sp 标题；品牌标记 24dp，产品名为 15sp；数字继续启用 tabular figures。底栏基准 60dp；字体缩放达到 1.6× 时改为横向图标+短标签，避免纵向图标、标签和系统导航区相互挤压。顶栏连接状态同时切换为短文案，完整事实保留在 TalkBack 描述和设置页。
- 导航：四个目的地为“今天／训练／记录／恢复”。今天只承担当前训练、阶段顺序和进入训练控制；计划库由“管理训练计划”按需进入，避免管理数据淹没每日主任务。计划详情和编辑器隐藏全局底栏，防止遮挡表单或误切页丢草稿；返回/保存由当前层明确承载。深色底栏与系统导航栏连续，使用浅色图标/标签和品牌红 3dp 选中指示条。
...
- API 35、1080×2400 专用模拟器已覆盖今天/训练/历史/睡眠、设备设置、计划库和编辑器；1.0/1.3/2.0 三档均无文字或系统栏重叠。2.0× 使用横向底栏，1.3× 编辑器的分段控件、输入、添加与保存均可滚动到达；详情/编辑器无全局底栏。截图保存在忽略的 `.gradle/simulation`，不进入 Git。
