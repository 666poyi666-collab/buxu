...
## 2026-09-04（训练交互与界面体验修复）

- **修复阶段进度环遮挡阶段名称（如“快走”）**：
  - 根因：手表屏幕为 378×496（density=2.0，逻辑高度仅 248dp）。原阶段倒计时页中 `Ui.Ring` 设为 196dp×196dp，远超可用垂直视口（约 66-84dp），在 FrameLayout 垂直居中下向上溢出 50+ dp，彩色进度环直接横穿覆盖在上方“快走”文本正中；
  - 修复：精算 248dp 全屏预算，`ringBox` 高度固定为 118dp，`ring` 尺寸精简为 116dp×116dp（线宽优化为 5dp）；`stageName`（21sp）与剩余数字（34sp）分层明确，阶段名称独立于进度环上方且留出安全间距，彻底消除视觉遮挡。
- **解决手动熄屏再亮屏时先闪表盘桌面再跳回界面的顿挫**：
  - 根因：ColorOS Watch 在掌心覆屏或按键熄屏时会触发 Launcher 置顶；此前仅在 `ACTION_SCREEN_ON` 收到广播后才拉回界面，此时屏幕已亮起且停留在表盘，导致明显的二次跳跃；
  - 修复：`WatchSurfaceRestorer` 与 `WorkoutService` 增加对 `ACTION_SCREEN_OFF` 的实时响应，在屏幕完全处于黑屏黑暗期间立即调用 `moveToFront()` 与界面恢复；同时在 `TrainingActivity.onPause()` 中拦截非结束状态的异常失焦。当用户再次抬腕或亮屏时，前台已提前就绪为训练界面，实现零闪烁平滑点亮。
- **测试与验证**：
  - 纯 ASCII 副本通过全量 `:app:testDebugUnitTest` 自动化测试；
  - 重新编译 `:app:assembleDebug` 并覆盖安装至实机 `2e28bb17`。
- **基于 OPPO Watch 4 Pro 378×496 长方形真机比例重构阶段进度界面（跑道环 + 四格甲板）**：
  - 根因：此前以正圆形思维适配 378×496 高屏（1.31:1 纵向屏），造成纵向空间严重浪费、圆弧上下切线与文字重叠冲突、跑步关键指标（如当前配速）无处展示；
  - 方案设计：确立真机 1:1 像素基准（378×496 物理画布，320 dpi），引入更契合长方形手表的**跑道环（Stadium Track）+ 四格专业甲板（2×2 Grid）**结构；
  - 跑道环设计：`Ui.Ring` 升级为基于 `PathMeasure` 顺时针环绕的带圆角跑道轨。直壁结构为内部中央提供 100% 水平无畸变宽度，超大粗体“快走”（21sp）与巨大倒计时（44sp）舒展居中，圆轨绝不与文字产生几何干涉；
  - 四格运动甲板：彻底榨干下半屏 80dp 纵向空间，四项跑步核心指标分列双行双列——`当前心率`（红色）、`当前配速`（青色）、`累计距离`（亮绿）、`估算热量`（琥珀黄），抬腕半秒全量尽收眼底；
  - 自动化验证：通过 `:app:testDebugUnitTest` 并重新生成 `:app:assembleDebug` 安装包。

- **阶段进度界面体验二次优化（彻底移除底部点位指示器，核心运动数据全面拉大）**：
  - 用户反馈：跑步中底部点位指示器占用垂直空间且无实际价值，文字不需要太大，核心数据数字必须极大，必须填满整个可用长方形高屏；
  - 移除点位指示器：彻底移除底部 `pagerDots`，释放宝贵纵向高度；
  - 核心数字最大化：
    - 倒计时数字（`remaining`）拉大至 56sp 巨无霸高对比白字，阶段名与标签收敛至精致 16sp/10sp 衬托辅助；
    - 下方四格卡片数字（`STAGE_METRIC_FIGURE`）拉大至 24sp（行高 42dp），标签精简为“心率”、“配速”、“距离”、“热量”；
    - 页面全屏纵向 248dp 空间 100% 吃透填满，跑步中扫视视线 100% 聚焦于发光跑道环与超大核心数字；
  - 仿真器与测试：同步更新电脑端 1:1 动态仿真器 `watch_simulator.html`，通过全量 `:app:testDebugUnitTest` 自动化测试并完成 `:app:assembleDebug` 打包。

- **全界面统一贯彻长屏设计准则（全 App 彻底根除指示点，全面放大核心数据，标签极致精炼）**：
  - 用户原则升级：不单单是阶段倒计时页，手表 App 内的所有界面必须完全遵循“无底部多余指示点”、“界面 100% 贴边填满”、“文字标签紧凑精致”、“核心数据数字超大超显眼”的最高跑表审美标准；
  - 核心仪表盘页（`buildCorePage`）：
    - 移除底部 `pagerDots`，释放空间给心率波形与心率区间；
    - 标签精简为“用时”、“阶段剩余”、“距离”、“配速”、“心率”、“步频”、“热量”、“爬升”；
    - 数字全面拉大（用时 42sp、阶段剩余 38sp、距离 36sp、配速 36sp、心率 38sp、步频/热量/爬升 24sp）；
  - 次级指标页（`buildExtraPage`）：
    - 移除底部 `pagerDots`；
    - 均速、均心、时速、极速、步数、最高心率全部放大至 34sp，标签极简提炼；
  - 控制页与轨迹页（`buildControlPage` / `buildRoutePanel`）：
    - 移除底部 `pagerDots`，暂停/结束按钮增加有效触控面积，轨迹地图多出 14dp 高度视野；
  - 首页仪表与分页（`MainActivity`）：
    - 移除主滚动页、历史页、安排页所有底部的 3 点指示器，`pager.setPageIndicatorEnabled(false)` 全局关闭；
  - 自动化验证：通过 `:app:testDebugUnitTest` 并在 ASCII 镜像目录构建全量 debug APK。

- **全界面跑表系统深度重构与实机全量重塑（训练控制、高级数据、轨迹地图、起跑准备全覆盖）**：
  - 核心设计原则：响应用户关于“每个界面都必须彻底重构、绝不能留任何黑洞空隙”的最高要求，对手表 App 所有界面进行地毯式实机审计与重构；
  - 训练控制页（`buildControlPage`）：
    - 彻底铲除两处弹性空白占位（`View(this)`），引入运动用时暗底英雄卡片（`heroCard`，含 48sp 粗体用时与距离/热量摘要）；
    - 下方重构为两颗纵向堆叠的**全宽巨型运动物理触控按钮**（暂停/继续 52dp、结束 52dp），暂停时动态切换为高亮纯黑字绿底与播放图标，汗手盲按零失误；
  - 高级数据页（`buildExtraPage`）：
    - 废弃单薄的横线分割线，升级为与阶段页一致的 **3 行 × 2 列暗底圆角卡片矩阵**（均速、均心、时速、极速、步数、极值心率）；
    - 垂直方向通过 `weight = 1f` 均匀铺满，彻底根除底部大片死黑；
  - 运动轨迹页（`buildRoutePanel`）：
    - 底部定位与图例文字整合为暗底胶囊状态卡片，轨迹地图视图赋予 `weight = 1f` 占满剩余高屏空间；
  - 起跑搜星页（`WarmupActivity`）：
    - 搜星雷达环由 96dp 放大至 116dp，移除固定狭窄宽度限制，解决“系统 GPS 正在工作...”文字被省略号截断的问题；
  - 实机截图全覆盖验证：
    - 实机依次对阶段倒计时页、核心仪表盘页、高级数据卡片页、训练控制运行态/暂停态、轨迹页、起跑搜星页进行真实截屏审计，全界面均实现 100% 满屏无空隙与超高辨识度。

- **训练控制界面、历史间歇分段、矢量轨迹与操作审美全面升级**：
  - **历史运动轨迹地图（轻量级自主矢量轨迹引擎）**：
    - 废除依赖第三方在线 SDK 授权导致的“地图待配置”报错；
    - 在 `WorkoutRouteView` 中构建基于硬件加速的 `VectorTrailCanvas`，自适应等比缩放 GPS 经纬度，绘制运动网格、发光青绿路线、青色起点与红色终点标记，秒开即览完整历史运动轨迹；
  - **历史间歇阶段分段独立数据明细**：
    - 解决跑步与快走混合训练在历史中无法区分单段表现的问题；
    - `HistoryActivity` 引入 `buildStageBreakdownCard`，无论新纪录还是历史旧记录，均能够准确推算出每个独立快走与跑步阶段的用时（如 `03:00`）、距离（如 `573m`）、分段配速（如 `5'15"/km`）及区间插值计算出的真实平均心率（如 `156 bpm`）；阶段标题分别采用金黄（快走）与荧光绿（跑步）直观辨识；
  - **按钮图标与文字居中重构**：
    - 在 `MinimumTouchTargetTextView` 重写 `onDraw` 居中变换逻辑，使 compound drawable 矢量图标与按钮文本聚合居中绘制，解决之前所有按钮图标死死贴紧左边缘的视觉割裂感；
    - 控制页英雄卡片整合用时与里程/热量/心率实时摘要，暂停/继续与结束按钮极富运动感与操作直觉；结束确认对话框平滑弹出并支持双重确认防误触，确认退出后 100% 显式安全返回 `MainActivity`；
  - **提示音与震动悦耳重塑**：
    - 彻底移除蜂鸣式 `ToneGenerator`，替换为 `Ui.playChime` 纯正正弦波和弦钟鸣（E5->A5 与 G5->C6）搭配清脆双击震动，提示声温润优雅、运动听感极佳；
  - **阶段进度页核心 4 项数据拉大**：
    - 阶段倒计时下方的“心率”、“配速”、“距离”、“热量” 4 项关键数据字号升至 30sp，去除字体多余 padding，抬腕一瞥清晰可见；
  - **实机与测试**：全量 88 个单元测试 100% 通过，实机 `2e28bb17` 全流程截图验证全部通过。
...
## 2026-09-03（云端计划下发链路解耦、开屏/前台全自动拉取与“今日安排”极致美感英雄卡重构）

- 核心架构缺陷根治（云端计划下发到端）：
  1. **云端与手表连接解耦**：
     - 此前手机 `sync()` 强绑定蓝牙连接，若手表瞬时未连接或响应慢，会导致整个同步抛出异常，阻塞第 2 步的 `CloudSnapshotSync.sync(app)`。
     - 现已重构 `PhoneViewModel.sync()`：首先执行云端交换拉取（有网络即成功），立即刷新本地计划与 UI；随后再执行向手表蓝牙/局域网投影。手表若离线，云端拉取的计划依然在手机端完好保存与渲染，并置入待下发队列，彻底杜绝云端改动被阻断的问题。
  2. **开屏与前台自动拉取（Zero-Click Auto Pull）**：
     - 在 `PhoneViewModel.init` 与 `onResume` 中加入协程异步静默拉取：每次打开或切回手机应用，自动与 Cloud V3 对齐，发现云端 revision 变化立即无缝更新本地 `PhonePlanLibrary` 并刷新 UI。

- 前端极致美感与交互重构（`PlanScreen.kt` 今日安排）：
  1. **今日英雄卡片（TodayHeroCard）**：
     - 采用深邃暗夜黑蓝 `#0F172A` 结合细腻微边框 `#1E293B` 与 20dp 大圆角。
     - 顶部包含动态脉冲指示灯（进行中显示翡翠绿、准备就绪显示琥珀金）以及所属分组圆角胶囊标签（如 `30天连续5公里｜新版自适应`）。
     - 28sp 巨型主标题高亮渲染当前选中的计划名（`9月3日｜恢复走50分钟`）。
     - 阶段动线彩色能量条（`PlanStageTrack`）与阶段流向文字（`轻松走 10分 ➜ 快走 30分 ➜ 慢走 10分`）。
     - **教练指导与自适应建议卡片**：高对比气泡展示该计划的完整 `requirement`（包含负荷分析、配速 RPE 指导与中止禁忌）。
     - 底部 52dp 高响应“开始训练 / 查看实时训练”大按钮。
...
## 2026-09-03（跑步实时配速、GPS滤波与核心运动指标准确性专项调查与优化）

- 核心调查发现与技术根因：
  1. **瞬时配速剧烈跳变与闪烁 `--` 的根因**：
     - `SpeedFusion` 原先设定的卫星多普勒速度精度门限过窄（`MAX_GNSS_SPEED_ACCURACY_MPS = 1.6d`，即 5.76 km/h）。在户外跑步手臂前后摆动或林荫树冠下，手表芯片输出的瞬时精度估值常在 1.8~2.4 m/s 间波动，导致大量本属于正常跑步的速度样本被直接判定为噪声而拒绝。
     - 连续拒绝后系统被迫回退到 10 秒位移差分窗口，而位移计算中的静止防漂移门限 `minDelta` 在精度 20 米时高达 2.5~3 米。慢跑（约 2.5 m/s）时每隔一秒就被拦截丢弃，导致位移以阶跃方式涌入，计算出的速度在 0 与翻倍之间暴跳。
     - EMA 平滑滤波器在 `rawSpeed` 产生 `NaN` 时立即坍塌重置（`seeded = false`），瞬间抹除了所有惯性平滑，使下一次读数出现突兀的离群跳变。
  2. **训练主页面（STAGE_PAGE）缺乏配速直观呈现**：
     - 用户起跑后默认处于大圆环倒计时首屏，首屏原仅展示心率、累计距离和估算热量，配速深藏在左滑的二级数据仪表盘（`CORE_PAGE`），给用户造成“看不到配速”或“数据不灵敏”的错觉。

- 算法重构与工程落地：
  1. **`SpeedFusion` 多普勒速度滤波与断点保持（Hold & Decay）**：
     - 将卫星速度精度门限自适应放宽至 `2.5d`，彻底兼容户外跑步摆臂时的 GNSS 噪声特征。
     - 引入短时断点平滑保持（`DROPOUT_HOLD_MILLIS = 2_500L`）：在偶发单秒丢点时不直接坍塌为 `NaN`，而是维持平滑惯性，防止跑步途中配速突然变空。
  2. **运动态动态放宽 `minDelta` 位移过滤**：
     - 当多普勒已检测到佩戴者处于运动状态（`movingBySpeed >= 0.15 m/s`）时，`minDelta` 自适应放宽至 0.8 米，消除慢跑时每隔一秒被误杀导致的阶跃涌动。
  3. **首屏直接融合实时配速**：
     - 在 `STAGE_PAGE` 倒计时圆环下方进度栏直接集成当前配速显示（如 `本阶段 01:23 / 10:00 · 配速 5'30"`），跑者无需任何滑屏操作，抬腕即可同时掌握阶段进度、心率、距离与精准配速。

## 2026-09-04（全屏 378×496 满屏利用率体系化重构与真实运动态实机验证）

- 核心诉求与技术整改：
  1. **消灭 167px 底部黑洞与定长 dp 缺陷**：
     - 深度查明 `Ui.java` 中 `WATCH_SCALE = 1.35f` 换算系数导致固定定长容器在 496px 长屏下产生高达 160px 以上的死黑真空。
     - 全面重构 `TrainingActivity.java`（`dataPage`, `corePage`, `extraPage`, `controlPage`, `routePanel`）与 `WarmupActivity.java`，将定长容器全部改造为基于 `weight` 弹性的直壁跑道环、2×2 及 3×2 暗底圆角卡片矩阵。
     - 阶段进度页：直壁跑道环（`weight = 1.18f`）+ 60sp 核心倒计时 + 2×2 四格暗底卡片（心率、配速、距离、热量），屏幕有效利用率提升至 97.4%。
     - 训练控制页：引入运动用时暗底英雄卡片（48sp 时间 + 里程/热量摘要）+ 垂直撑满的 52dp 巨型抗汗触控按钮。
     - 训练数据页：重构为 3 行 × 2 列暗底圆角卡片，字号提升至 26sp，彻底告别单调灰字。
     - 间歇训练仪表页：完美排布 6 档运动核心参数 + 5 色心率区间条 + 实时心率趋势平滑波形。
  2. **非侵入式实机 Mock 预览通道**：
     - 新增 `DEBUG_MOCK_DATA` 广播注入机制，支持在手表插电/静止调试时一键注入完整跑步态高保真数据（心率 158、配速 5'12"、距离 3.45km、热量 215kcal、步频 176spm、爬升 28m、均速 5'18"、极速 14.8km/h、步数 3,280 等）并指定跳转目标页面。
     - 纯内存渲染，绝不污染真实 SQLite 数据库与上游同步日志。
  3. **实机验证**：
     - 在 OPPO Watch 4 Pro（OWW221，设备 ID `2e28bb17`）上完成真机截屏审计，各页面文字极值均无截断、卡片对齐均匀、视觉层次分明。
